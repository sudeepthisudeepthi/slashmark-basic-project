...
tasks = pd.DataFrame(columns=['description', 'priorty'])
...
def save_tasks():
    tasks.to_csv('tasks.csv', index=True)
...
# If there are tasks available for training
if not tasks.empty:
    # Fit the model only if there are tasks
